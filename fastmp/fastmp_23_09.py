...
class fastMP:
    ...
    def fit(self, X):
        ...
        for N_clust in range(self.N_membs_min, self.N_membs_max):
            ...
            d_idxs, d_sorted, d_pm_plx_idxs, d_pm_plx_sorted = self.getDists(
                all_data, PMsPlx_data, xy_c, vpd_c, plx_c)

            # Most probable members given their distances
            st_idx = self.getStars(  # N_survived
                xy, d_pm_plx_idxs, d_pm_plx_sorted, N_clust, lon, lat, pmRA, pmDE, plx)
    ...
    def getDists(self, all_data, PMsPlx_data, xy_c, vpd_c, plx_c):
        ...
        return d_idxs, d_sorted, d_pm_plx_idxs, d_pm_plx_sorted

    def getStars(self, xy, d_idxs, d_sorted, N_clust, lon, lat, pmRA, pmDE, plx):
        """
        """
        ...
        # plt.subplot(131)
        # plt.scatter(lon[msk], lat[msk], c='grey', alpha=.5)
        # plt.subplot(132)
        # plt.scatter(pmRA[msk], pmDE[msk], c='grey', alpha=.5)
        # plt.subplot(133)
        # plt.hist(plx[msk], color='grey', alpha=.5, density=True)
        # plt.show()
        ...
        for step in np.arange(N_clust, N_stars, N_clust):
            ...
            # if step_old == 0:
            #     idx_survived += list(d_idxs[:step])
            #     step_old = step
            #     continue

            # Break condition
            d1_median = np.median(d_sorted[step_old:step])
            d1_MAD = MAD(d_sorted[step_old:step])
            d2_median = np.median(d_sorted[step:step + N_clust])
            d2_MAD = MAD(d_sorted[step:step + N_clust])

            temp.append([step, d1_median, d1_MAD, d2_median, d2_MAD])

            # Break condition
            # if d_avrg > ld_median + self.N_std_d * ld_std:

                # print("break", N_clust, step_old, step, d_avrg, ld_median, ld_std)
                # msk1, msk2 = d_idxs[:step_old], d_idxs[step_old:step]
                # plt.figure(figsize=(12, 4))
                # plt.suptitle(f"{N_clust}, {step}")
                # plt.subplot(131)
                # plt.scatter(lon[msk1], lat[msk1], c='grey', alpha=.5)
                # plt.scatter(lon[msk2], lat[msk2], c='r', alpha=.5)
                # plt.subplot(132)
                # plt.scatter(pmRA[msk1], pmDE[msk1], c='grey', alpha=.5)
                # plt.scatter(pmRA[msk2], pmDE[msk2], c='r', alpha=.5)
                # plt.subplot(133)
                # plt.hist(plx[msk1], color='grey', alpha=.5, density=True)
                # plt.hist(plx[msk2], color='r', alpha=.5, density=True)
                # plt.show()

                # break

            # idx_survived += list(d_idxs[step_old:step])
            step_old = step
        ...
        # plt.plot(temp[0], temp[3], c='b')
        # plt.subplot(122)
        plt.plot(temp[0], temp[1] + 3 * temp[2], c='r')
        plt.plot(temp[0], temp[3] - 3 * temp[4], c='b')
        plt.show()
        # breakpoint()
        # print(len(idx_survived))
